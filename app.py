...
def get_cart_items():
    ...
    return get_db().execute(
        ...
        f"JOIN vendors ON vendors.id = products.vendor_id WHERE products.id IN ({marks})",
        ids,
    ).fetchall()
...
def shared_template_data():
    return {"cart_count": len(session.get("cart", [])), "current_user": session.get("user_name")}
    ...
    if request.method == "POST":
        ...
        if action == "register":
            try:
                ...
                db.commit()
                session.update(user_id=cursor.lastrowid, user_name=name)
    ...
    if request.method == "POST":
        ...
        if not name or not email:
            ...
            try:
                ...
                flash("Your vendor account is ready. Add your first product!", "success")
    ...
    if request.method == "POST":
        ...
        for item in items:
            db.execute(
                ...
                (session["user_id"], item["id"], "confirmed"),
            )
        db.commit()
        session["cart"] = []
